...
def load_backup_data():
    ...
    try:
        # Try simplified backup first, then full backup
        filename = 'simplified_backup.json' if os.path.exists('simplified_backup.json') else 'lda_data_backup.json'
        with open(filename, 'r') as f:
            data = json.load(f)
        
        print(f"✅ Backup data loaded successfully from {filename}")
